...
create table CUSTOMERS (expirationdate CHAR(18) CHECK(LENGTH(EXPIRATIONDATE) >= 3) not null, id bigint generated by default as identity, cardnumber CHAR(18) CHECK(LENGTH(CARDNUMBER) >= 20) not null, phone CHAR(18) CHECK(LENGTH(PHONE) >= 9) not null, address CHAR(18) CHECK(LENGTH(ADDRESS) >= 100) not null, fullname VARCHAR(150)  CHECK(LENGTH(FULLNAME) >= 10) not null, email varchar(255), primary key (id));
...
create table Products (DISCOUNTS numeric(3,2) not null, PRICES numeric(6,2) not null, STOCK integer not null check (STOCK>=0), FAMILY_ID bigint not null, id bigint generated by default as identity, name VARCHAR(150)  CHECK(LENGTH(NAME) >= 10) not null, description VARCHAR(150)  CHECK(LENGTH(DESCRIPTION) >= 10) not null, IMAGES_URL varchar(2083) not null, primary key (id));
create table PRODUCTS_RATINGS (PRODUCT_ID bigint not null, id bigint generated by default as identity, RATES VARCHAR(50) DEFAULT 'RATES' not null, primary key (id));
create table PROVIDER_FAMILY (FAMILY_ID bigint not null, PROVIDER_ID bigint not null);
create table PROVIDERS (id bigint generated by default as identity, providerPhone CHAR(18) CHECK(LENGTH(PROVIDERPHONE) >= 9) not null, providerCif VARCHAR(30) CHECK(LENGTH(PROVIDERCIF) >= 9), providerAddress CHAR(18) CHECK(LENGTH(PROVIDERADDRESS) >= 100) not null, providerName VARCHAR(150) CHECK(LENGTH(PROVIDERNAME) >= 10) not null, PROVIDERS_EMAIL varchar(255), primary key (id));
...
alter table if exists PRODUCTS_RATINGS add constraint FKopnw5w8bnusgt8pdwqkpayrom foreign key (PRODUCT_ID) references Products;
alter table if exists PROVIDER_FAMILY add constraint FKl3l7cat7fw9v49kh1apciaaum foreign key (FAMILY_ID) references FAMILY_OF_PRODUCTS;
alter table if exists PROVIDER_FAMILY add constraint FKs4s6n8i60p4132u0yvp6xt3ia foreign key (PROVIDER_ID) references PROVIDERS;
